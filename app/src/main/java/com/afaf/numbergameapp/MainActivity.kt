package com.afaf.numbergameapp
...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var buttonGuss: Button
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        textGuess = findViewById(R.id.etGuess)
        buttonGuss = findViewById(R.id.btGuess)

        buttonGuss.setOnClickListener { addMessage() }
    }

    private fun addMessage(){
        val msg = textGuess.text.toString()
        if(msg.isNotEmpty()){
            if(guesses>0){
                if(msg.toInt() == answer){
                    disableEntry()
            ...
            textGuess.text.clear()
            textGuess.clearFocus()
    ...
    private fun disableEntry(){
        buttonGuss.isEnabled = false
        buttonGuss.isClickable = false
        buttonGuss.isEnabled = false
        buttonGuss.isClickable = false
    }
